...
- **Planetoid Simulation**:
  - Planetoid properties (including moons) are procedurally generated by the `usePlanetData` hook (`src/app/components/scene3d/usePlanetData.ts`), which utilizes texture generation and randomization functions from `src/app/components/scene3d/utils.tsx`.
  - The `Moon` component (`src/app/components/scene3d/Moon.tsx`) is responsible for rendering individual moons.
  - Physics (gravity, collisions) are managed by Rapier. The core simulation loop and application of forces are handled by the `usePhysicsSimulation` hook (`src/app/components/scene3d/usePhysicsSimulation.ts`), which updates the `RigidBody` components representing each planetoid.
