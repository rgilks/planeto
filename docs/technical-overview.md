...
- **`src/app/components/Scene.tsx`**: The main 3D scene component.
  ...
  - Utilizes helper functions from `src/app/components/scene/utils.tsx` for tasks like geometry definition.
  - Uses `src/app/components/scene/Moon.tsx` for rendering planetary moons.
- **`src/app/components/Eyes.tsx`**: Renders representations of other users' eyes ("eyes").
...
- **`src/lib/store/`**: Zustand stores for managing client-side state:
  ...
  - `useCamStore.ts`: Manages data for remote eyes, populated by `useEyes.ts`.
...
- **Planetoid Simulation**:
  - Planetoid properties (including moons) are procedurally generated by the `usePlanetData` hook (`src/hooks/usePlanetData.ts`), which utilizes texture generation and randomization functions from `src/app/components/scene/utils.tsx`.
  - The `Moon` component (`src/app/components/scene/Moon.tsx`) is responsible for rendering individual moons.
...
- **Symbol Event Broadcasting & Remote Display**:
  - `Scene.tsx` observes `lastInput` from `useSymbolStore`. On change, it POSTs a `SymbolEvent` (containing user ID and key) to `/api/events`.
  - The server (`/api/events/route.ts`) validates and broadcasts this event via SSE to all connected clients.
  - Remote clients receive these `SymbolEvent`s via their SSE connection in `Scene.tsx`.
  - `useSymbolStore` on remote clients is updated with the key press from the other user.
  - `Eyes.tsx` on remote clients observes `useSymbolStore` and displays the received symbol near the originating user's eye representation.
- **Eye Position Sharing**:
  - `useEyePublisher.ts` (used within `Scene.tsx`, located in `src/hooks/useEyePublisher.ts`) sends the local eye's position as a `EyeUpdateEvent` to `/api/events` (POST). This occurs periodically or on significant movement.
  ...
  - Remote clients receive these updates via `useEyes.ts` (located in `src/hooks/useEyes.ts`), which populates `useCamStore.ts`.
  - `Eyes.tsx` uses `useCamStore` (via `useEyes`) to position the visual representations of remote users.
