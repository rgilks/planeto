...
- Multiplayer state lives in **Durable Objects** (one `EventsChannel` per room, capped at 30, plus a `RoomDirector` that assigns rooms); the app is otherwise a static export. `/api/events` only runs under `wrangler dev` (`npm run preview`), not `next dev`. Read the Gotchas in AGENTS.md before touching the Worker, the DOs, or deploy config.
